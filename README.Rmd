...
The goal of `trackr` is to take away this pain by instrumenting and monitoring a dataframe through a `dplyr` pipeline, creating a step-by-step summary of the 
...
want to prepare a stratified data set that excludes flowers with narrow petals, and those with the biggest petals of each Species. With `trackr` 
we can mix regular `dplyr` commands with additional `trackr` commands such as `comment` and `status`, and an enhanced implementation 
...
It is usually at this point someone says, "Did you exclude XYZ?" or "How many of WXY did you find in each group?" or something similar. Before trackr you 
