...
For installation from source on in Linux, `dtrackr` has required transitive dependencies on 
...
For automated testing `dtrackr` uses the `testthat` framework. For vignette
building there are dependencies on the `tidyverse` package, as well as other 
system libraries required for vignette building with `pandoc`.
